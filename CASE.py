...
def createAoACase(filename, casename, velocity, mass, AoA, flaps=True, ailerons=True, elevator=True, rudder=True, rho=1.225, Ixx=0.1, Iyy=0.1, Izz=0.04):
    with open(filename, 'a') as f:
        f.truncate(0)
        f.write(f"---------------------------------------------\nRun case  1:  {casename}\n")
        f.write(f"alpha        ->  alpha          =   {AoA}\n")
        f.write("beta         ->  beta        =   0.00000\n")
        f.write("Flaps        ->  Flaps       =   0.00000\n") if flaps else None
        f.write("Rudder       ->  Rudder =   0.00000\n\n") if rudder else None
